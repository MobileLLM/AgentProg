...
def _find_adb_directory() -> str:
  ...
  potential_paths = [
      f"{os.environ['ANDROID_SDK_ROOT']}/platform-tools/adb",
...
def _get_agent(
  ...
  elif _AGENT_NAME.value == 't3a_gemini':
    agent = t3a.T3A(env, infer.GeminiGcpWrapper('gemini-2.5-pro'))
  elif _AGENT_NAME.value == 'm3a_gpt4v':
    agent = m3a.M3A(env, infer.Gpt4Wrapper('gpt-4o'))
  elif _AGENT_NAME.value == 'm3a_gemini':
    agent = m3a.M3A(env, infer.GeminiGcpWrapper('gemini-2.5-pro'))
  elif _AGENT_NAME.value == 'ariaui':
    agent = aria_ui.Ariaui(env,infer.Gpt4Wrapper('gpt-4o'))
  elif _AGENT_NAME.value == 'uvg':
    agent = uvg.UVG(env, infer.Gpt4Wrapper('gpt-4o'), grounding_model_name="UGround-V1-7B")
  ...
  elif _AGENT_NAME.value in ['agentprog', 'agentprog_w_o_belief_state', 'agentprog_w_aw_env']:
    from android_world.agents import agentprog
    agent = agentprog.AgentProg(env, name=_AGENT_NAME.value)
  elif _AGENT_NAME.value == 'stupid_agent':
    agent = stupid_agent.StupidAgent(env)
