...
_PATTERN_TO_ACTIVITY = immutabledict.immutabledict({
    ...
    'files|file manager': 'com.google.android.documentsui/com.android.documentsui.files.FilesActivity',
    ...
    'simple sms messenger|simple sms|sms messenger|sms|messager': 'com.simplemobiletools.smsmessenger/com.simplemobiletools.smsmessenger.activities.MainActivity',
...
def close_recents(env: env_interface.AndroidEnvInterface):
    ...
    # 使用正则提取所有任务块，例如 Task{...} 之间的内容
    tasks = re.findall(r'(Task\{[^\}]+\})', output)
    for task in tasks:
        # 如果任务块中包含要排除的包名，则不清理
        # 从任务块中提取出 id
        recents_id_match = re.search(r'id=(\d+)', task)
        if recents_id_match:
            recents_id = recents_id_match.group(1)
            # print("清理任务, recents_id: ", recents_id)
            issue_generic_request(['shell', 'am', 'stack', 'remove', recents_id], env)
    # print("----- close_recents end -----")
