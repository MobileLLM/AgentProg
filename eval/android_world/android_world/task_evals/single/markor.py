...
class MarkorMergeNotesLong(Markor):
  """Task for merging ten existing notes into a new one."""

  complexity = 100
  schema = {
      "type": "object",
      "properties": {
          **{f"file{i}_name": {"type": "string"} for i in range(1, 11)},
          "new_file_name": {"type": "string"},
          **{f"file{i}_content": {"type": "string"} for i in range(1, 11)},
      },
      "required": [
          *[f"file{i}_name" for i in range(1, 11)],
          "new_file_name",
          *[f"file{i}_content" for i in range(1, 11)],
      ],
  }
  template = (
      "Merge the contents of Markor notes " + 
      ", ".join([f"{{file{i}_name}}" for i in range(1, 10)]) + 
      f" and {{file10_name}} (in the same order) into a new Markor note named"
      " {new_file_name} and save it. Add a new line between the content of each"
      " note."
  )
  ...
  def __init__(self, params: dict[str, Any]):
    ...
    super().__init__(params)
    merged_content = "\n\n".join([
        self.params[f"file{i}_content"] for i in range(1, 11)
    ]) + "\n"
  ...
  def initialize_task(self, env: interface.AsyncEnv) -> None:
    ...
    for i in range(1, 11):
      file_utils.create_file(
          self.params[f"file{i}_name"],
          device_constants.MARKOR_DATA,
          env.controller,
          content=self.params[f"file{i}_content"],
      )
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    # merged_file should have 19 lines: 10 content lines + 9 blank lines
    content_split = merged_file.split("\n")
    are_notes_merged = (
        len(content_split) == 19
        and all(not content_split[i] for i in range(1, 19, 2))  # Check blank lines at odd indices
    )
    return 1.0 if are_notes_merged else 0.0
  ...
  def generate_random_params(cls) -> dict[str, str | int]:
    ...
    for i in range(1, 11):
      params[f"file{i}_name"] = _generate_random_note().name
      params[f"file{i}_content"] = user_data_generation.generate_random_string(20)
    return params

class MarkorMergeNotesSuperLong(Markor):
  """Task for merging twenty existing notes into a new one."""

  complexity = 100
  schema = {
      "type": "object",
      "properties": {
          **{f"file{i}_name": {"type": "string"} for i in range(1, 21)},
          "new_file_name": {"type": "string"},
          **{f"file{i}_content": {"type": "string"} for i in range(1, 21)},
      },
      "required": [
          *[f"file{i}_name" for i in range(1, 21)],
          "new_file_name",
          *[f"file{i}_content" for i in range(1, 21)],
      ],
  }
  template = (
      "Merge the contents of Markor notes " + 
      ", ".join([f"{{file{i}_name}}" for i in range(1, 20)]) + 
      f" and {{file20_name}} (in the same order) into a new Markor note named"
      " {new_file_name} and save it. Add a new line between the content of each"
      " note."
  )
  ...
  def __init__(self, params: dict[str, Any]):
    ...
    super().__init__(params)
    merged_content = "\n\n".join([
        self.params[f"file{i}_content"] for i in range(1, 21)
    ]) + "\n"
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    # merged_file should have 39 lines: 20 content lines + 19 blank lines
    content_split = merged_file.split("\n")
    are_notes_merged = (
        len(content_split) == 39
        and all(not content_split[i] for i in range(1, 39, 2))  # Check blank lines at odd indices
    )
    return 1.0 if are_notes_merged else 0.0
